...
### ✅ 認証システム
- 会員登録（名前、受注番号、メールアドレス、パスワード、お祝いメッセージ）
- ログイン・ログアウト機能
- JWT認証とセッション管理
- パスワードのbcryptjs暗号化
...
- `message` - お祝いメッセージ内容（20字以内・URLエンコード推奨）
...
# お祝いメッセージ付き（URLエンコード・20字以内）
/register?order_id=ORD001&name=田中太郎&message=%E3%81%94%E7%B5%90%E5%A9%9A%E3%81%8A%E3%82%81%E3%81%A7%E3%81%A8%E3%81%86%E3%81%94%E3%81%96%E3%81%84%E3%81%BE%E3%81%99
...
3. 画像をアップロード（最大5枚・JPG自動変換）
4. お祝いのメッセージを入力（20字以内・任意）
5. 登録完了後、自動的にダッシュボードにリダイレクト
...
### 🆕 お祝いメッセージ機能
- **実装日**: 2025-09-08
- **仕様**: 20字以内のお祝いメッセージ入力機能
- **機能詳細**:
  - 結婚、出産、新築、開店などのお祝いに使用
  - リアルタイム文字数カウント表示
  - 文字数に応じた色分け表示（15字以上でオレンジ、20字で赤）
  - 20字制限の自動カット機能
  - 画像アップロード後の専用セクション配置
  - URLパラメーター対応（`message`パラメーター）
